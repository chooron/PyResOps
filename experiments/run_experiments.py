...
from experiments.paper_experiment_runner import AgnoMCPExperiment, SCENARIOS
...
def run_full_experiment(model_profile: str | None = None):
    ...
    mcp_experiment = AgnoMCPExperiment(model_profile=model_profile)
    ...
    for scenario in SCENARIOS:
        ...
        try:
            ...
            print(f"  综合得分 - 人工: {evaluation['human_overall']:.4f} | MCP: {evaluation['mcp_overall']:.4f}")
            print(f"  防洪评分 - 人工: {evaluation['human_safety']:.4f} | MCP: {evaluation['mcp_safety']:.4f}")
            print(f"  生态评分 - 人工: {evaluation['human_eco_score']:.4f} | MCP: {evaluation['mcp_eco_score']:.4f}")
            print(f"  约束违反 - 人工: {evaluation['human_violations']} | MCP: {evaluation['mcp_violations']}")
        except Exception as e:
            print(f"  ✗ 评估失败: {e}")
...
def run_paper_experiments(model_profile: str | None = None) -> dict:
    """
    论文实验入口：静态基线 + 多轮动态对比（5场景 × 3轮）。

    结果目录：
      experiments/results/static/   — 无触发基线（5个文件）
      experiments/results/dynamic/  — 多轮触发结果（5场景 × 3轮 + 5个汇总）
    ...
    print("=" * 60)
    print("PyResOps 论文实验（静态基线 + 多轮动态对比）")
    print("=" * 60)
    print(f"实验时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"模型配置: {model_profile or '默认（config.yml default_profile）'}")
    print(f"场景: S01~S05，每场景 3 轮（1/2/3 次触发）")

    print("\n[Step 1/2] 运行静态基线（无触发）...")
    static_results = run_all_static_baselines(model_profile=model_profile)

    print("\n[Step 2/2] 运行多轮动态实验（3轮触发）...")
    dynamic_results = run_all_multi_round(max_rounds=3, model_profile=model_profile)

    # 生成论文汇总对比表
    paper_table = []
    dynamic_map = {r["scenario_id"]: r for r in dynamic_results if "scenario_id" in r and "error" not in r}
    static_map = {r["scenario_id"]: r for r in static_results if "scenario_id" in r and "error" not in r}
    ...
    for sid in ["S01", "S02", "S03", "S04", "S05"]:
        ...
        for n in range(1, 4):
            rnd = dynamic.get("rounds", {}).get(f"round_{n}", {})
            row[f"round{n}_score"] = rnd.get("final_score", None)
            row[f"round{n}_improvement"] = rnd.get("score_improvement", None)
            row[f"round{n}_constraint_rate"] = rnd.get("final_constraint_rate", None)
            row[f"round{n}_tool_calls"] = rnd.get("total_tool_calls", None)
    ...
    # 打印对比表
    print("\n" + "=" * 60)
    print("论文对比表（静态基线 vs 多轮动态）")
    print("=" * 60)
    header = f"{'场景':<6} {'静态基线':>8} {'Round1':>8} {'Round2':>8} {'Round3':>8}"
    print(header)
    print("-" * len(header))
    for row in paper_table:
        s = row.get("static_score")
        r1 = row.get("round1_score")
        r2 = row.get("round2_score")
        r3 = row.get("round3_score")
        fmt = lambda v: f"{v:.4f}" if v is not None else "  N/A  "
        print(f"{row['scenario_id']:<6} {fmt(s):>8} {fmt(r1):>8} {fmt(r2):>8} {fmt(r3):>8}")
