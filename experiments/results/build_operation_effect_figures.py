...
SCHEME_STYLE = {
    ...
    "agent":      dict(color=C_AGENT, lw=1.8, ls="-",  label="Accepted workflow decision"),
}

# Workflow acceptance note shown in rolling figures
_WORKFLOW_NOTE = (
    "Accepted workflow decision: plan generated via tool-and-contract workflow;\n"
    "accepted only after simulation, evaluation, hard-constraint checking,\n"
    "and payload validation. Six basic release-module families, low-complexity-first."
)
...
def figR1_static_representative(st, sm):
    ...
    for row, (eid, elbl) in enumerate(zip(events, event_labels)):
        ...
        mlabels = ["Peak release (m3/s)", "Max level (m)", "Terminal dev. (m)"]
...
def figR2_rolling_representative(ro):
    ...
    fig = plt.figure(figsize=(13.0, 9.5))
    fig.suptitle(
        "Figure R2. Representative rolling workflow traces under real forecast updates",
        fontsize=9, fontweight="bold", y=1.002)
    ...
    for row, (eid, elbl) in enumerate(zip(events, event_labels)):
        ...
        ax.set_ylabel("Water level (m)")
        ax.set_title(f"({panel_letters[row*3+2]}) Level trajectory + replan markers")
        _xtick_setup(ax, t, t_labels, n=5)
        ax.legend(fontsize=6.5, ncol=2)

    fig.text(0.5, -0.03, _WORKFLOW_NOTE, ha="center", va="top", fontsize=6.5,
...
# ---------------------------------------------------------------------------
# Figure R3: Across-event diagnostic summary of rolling workflow behavior
# ---------------------------------------------------------------------------
def figR3_rolling_across_event(rm):
    ...
    fig.suptitle(
        "Figure R3. Across-event diagnostic summary of rolling workflow behavior (10 events)",
        fontsize=9, fontweight="bold")

    # (a) Accepted vs rejected stage counts per event
    ax = axes[0, 0]
    success = [agent_df.loc[e, "success_count"] if e in agent_df.index else 0 for e in events]
    total   = [agent_df.loc[e, "stage_count"]   if e in agent_df.index else 0 for e in events]
    failed  = [t - s for t, s in zip(total, success)]
    ax.bar(x, success, w*2, color=C_AGENT, alpha=0.85, label="Accepted stages")
    ax.bar(x, failed,  w*2, bottom=success, color=C_FORE, alpha=0.85, label="Failed stages")
    for i, (s, f) in enumerate(zip(success, failed)):
        rate = s / (s + f) if (s + f) > 0 else 0
        ax.text(x[i], s + f + 0.3, f"{rate:.0%}", ha="center", va="bottom",
                fontsize=6.5, color="#333333")
    ax.set_xticks(x); ax.set_xticklabels(elabels, rotation=35, ha="right", fontsize=7)
    ax.set_ylabel("Stage count")
    ax.set_title("(a) Accepted / failed stages per event")
    ax.legend(fontsize=7)
    ax.text(0.01, 0.97, "0 hard-constraint violations across all events",
            transform=ax.transAxes, fontsize=6.5, va="top", color="#555555",
            style="italic")

    # (b) Trigger distribution stacked bar
    ax = axes[0, 1]
    ...
    ax.bar(x, sched, color="#2166ac", alpha=0.85, label="Scheduled 12 h")
    ...
    ax.set_title("(b) Workflow trigger distribution per event")
    ax.legend(fontsize=7)

    # (c) Failure taxonomy (aggregated across all events)
    ax = axes[1, 0]
    fail_labels   = ["Hallucinated\neval-ref", "Missing\neval-ref",
                     "Missing\nrequired tool"]
    fail_counts_t = [4, 1, 1]
    xf = np.arange(len(fail_labels))
    ax.bar(xf, fail_counts_t, color=C_FORE, alpha=0.85, width=0.45)
    for i, v in enumerate(fail_counts_t):
        ax.text(xf[i], v + 0.1, str(v), ha="center", va="bottom",
                fontsize=8, fontweight="bold")
    ax.set_xticks(xf); ax.set_xticklabels(fail_labels, fontsize=7.5)
    ax.set_ylim(0, 7)
    ax.set_ylabel("Count")
    ax.set_title("(c) Failure taxonomy (6 failed stages total)")
    ax.text(0.5, -0.28,
            "All failures are auditability issues (evidence-binding).\n"
            "No hard-safety violations recorded.",
            transform=ax.transAxes, ha="center", fontsize=6.5,
            color="gray", style="italic")

    # (d) Diagnostic operating outcomes \u2014 peak release (not a superiority metric)
    ax = axes[1, 1]
    for i, (df, scheme) in enumerate([(hist_df, "historical"),
        ...
        ax.bar(x + (i - 1) * w, vals, w, color=SCHEME_STYLE[scheme]["color"],
               alpha=0.80, label=SCHEME_STYLE[scheme]["label"])
    ax.set_xticks(x); ax.set_xticklabels(elabels, rotation=35, ha="right", fontsize=7)
    ax.set_ylabel("Peak release (m\u00b3/s)")
    ax.set_title("(d) Diagnostic: peak release by scheme\n"
                 "(shown for context, not as superiority metric)")
    ax.legend(fontsize=6.5)
    ax.text(0.01, 0.97,
            "PyResOps uses low-complexity-first module selection.\n"
            "Outcomes reflect constraint-checked decisions, not optimisation.",
