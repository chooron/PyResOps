# A Workflow-Constrained LLM-Assisted Methodology for Auditable Single-Reservoir Flood Operation: The PyResOps Case Study

## Abstract

Reservoir flood operation requires decisions that are executable, constraint-checked, and auditable. Large language model (LLM) agents can interpret operator instructions and coordinate external tools, but unconstrained text generation cannot provide reliable support for safety-relevant reservoir operation. This paper proposes a workflow-constrained methodology for LLM-assisted auditable single-reservoir flood operation. The methodology embeds LLM agents as instruction interpreters and workflow coordinators, while deterministic tools perform hydrological computation, release simulation, constraint checking, and evaluation. A decision record is accepted only when the required workflow is completed, the final payload is bound to the current tool-returned evaluation result, schema validation succeeds, and hard constraints are satisfied. The methodology is instantiated as PyResOps and evaluated on retained flood events from Tankeng Reservoir, covering static planning, dynamic retain/replan operation, command intervention, rolling forecast-triggered operation, forecast-error audit, cross-executor testing, and tools/skills ablation. In the 462-record main workflow set, three LLM executors achieved acceptance rates of 98.9%, 97.6%, and 97.8%, with no hard-constraint or downstream-release violations. Ablation results show that tool access alone is insufficient; auditable LLM-assisted reservoir operation requires explicit workflow protocols and fail-closed validation.

**Keywords:** reservoir operation; LLM agent; workflow-constrained execution; auditable decision support; fail-closed validation; expert system; flood-control operation

---

# 1. Introduction

Reservoir flood operation is a safety-relevant engineering decision process. During a flood event, operators must interpret reservoir level, storage, inflow, release capacity, downstream constraints, forecast uncertainty, and operational targets, and then issue release decisions that can be executed over the remaining horizon. Unlike text-centered decision-support tasks, a reservoir-operation recommendation is not useful merely because it is fluent or plausible. It must correspond to a release trajectory that can be computed, simulated, checked against hard constraints, evaluated against operational objectives, and inspected after the event. This makes reservoir flood operation a demanding test case for the use of large language models in engineering decision support.

Conventional reservoir-operation systems rely on operating rules, rule curves, simulation models, optimization routines, and rolling-horizon control. These approaches remain essential because they preserve an explicit relationship between reservoir states, release decisions, physical constraints, and evaluation metrics. However, real-time flood operation is not only a static optimization problem. Operators may revise instructions during an ongoing event, such as lowering the terminal water-level target, tightening a release cap, shortening the deadline for reaching a target level, imposing a conservative buffer around an upper operating level, or deciding whether a forecast deviation requires replanning. Deterministic models can evaluate these requests once they are properly encoded, but they do not naturally provide a flexible interface for translating changing natural-language operational instructions into structured, executable, and auditable workflow tasks.

LLM agents provide a possible interface layer for this problem. They can interpret natural-language instructions, organize intermediate steps, select external tools, and generate structured summaries. This creates an opportunity to rethink how LLMs may enter reservoir-operation workflows. The relevant question is not whether an LLM can directly produce a plausible release recommendation. In a safety-relevant engineering setting, the more important question is whether an LLM agent can be embedded into the operational workflow in a way that keeps every accepted decision inside a verifiable computational process.

Direct use of LLM-generated recommendations is risky for reservoir operation. An agent may skip a required simulation, call tools in the wrong order, reuse stale values from a previous stage, report a result that was not produced by the current tool session, or generate a final answer that looks structured but is not linked to any reproducible evaluation. These errors are not only language-quality problems. They affect whether a decision record can be reconstructed and trusted after the event. For flood-control operation, a safe-looking answer without a valid execution trace is not an acceptable operational record.

This paper therefore proposes a workflow-constrained methodology for LLM-assisted auditable single-reservoir flood operation. The methodology defines a bounded role for the LLM agent. The agent acts as an instruction interpreter and workflow coordinator, while hydrological computation, release simulation, constraint checking, and performance evaluation are performed by deterministic reservoir-operation tools. Operational tasks are represented as explicit workflows, including static planning, dynamic retain/replan operation, mid-event command intervention, and rolling forecast-triggered operation. The agent may coordinate these workflows, but the final decision is accepted only when it is tied to a valid tool execution trace and passes external validation.

The methodology contains four core design principles. First, numerical reservoir-operation quantities should be grounded in deterministic tools rather than generated by the language model. Second, the agent should operate under workflow protocols that specify required tool sequences, branch logic, prohibited calls, and final payload requirements. Third, each final decision record should bind to the current-stage evaluation result returned by the tools, so that the reported decision can be traced to reproducible computation. Fourth, validation should be fail-closed: if a required tool call, evidence reference, schema field, or hard-constraint check is missing or invalid, the record is rejected rather than treated as a usable recommendation.

In this study, the methodology is instantiated as PyResOps for single-reservoir flood operation. PyResOps connects a deterministic reservoir-operation kernel with tool-mediated LLM-agent execution, workflow skills, and an external validator. The deterministic kernel computes release plans, state trajectories, evaluation metrics, and constraint checks. The tool interface exposes these computations to the agent through structured inputs and outputs. Workflow skills specify how static, dynamic, and rolling operation tasks must be executed. The validator then checks tool order, evidence binding, payload schema, hard operational constraints, and downstream release constraints before accepting a decision record.

The case study uses retained flood-event records from Tankeng Reservoir. The evaluation is organized as a staged validation design. Stage 1 directly calls the deterministic reservoir-operation kernel to establish executable references. Stage 2 exposes the same computations through tool-mediated fixed workflows to verify that tool schemas and workflow wrappers reproduce the deterministic references. Stage 3 introduces LLM executors and evaluates whether they can complete auditable operation records under workflow constraints. The experiments cover a 462-record main workflow set, a 492-record static instruction-conditioned matrix, a 40-record command-intervention subset, a 373-check rolling workflow, a 51-stage forecast-error audit, and a tools/skills ablation.

This design shifts the unit of evaluation from a free-form recommendation to a decision record. A record includes the task context, tool trace, evaluation reference, structured payload, validation outcome, and failure reason if rejected. This record-level perspective is necessary because LLM-agent failures may occur at the workflow or evidence-binding level even when the underlying reservoir computation is feasible. A rejected record is therefore not necessarily a poor hydrological plan; it may be a plan that cannot be accepted because the agent failed to complete the required auditable procedure.

The contributions of this paper are threefold. First, it proposes a workflow-constrained methodology for embedding LLM agents into single-reservoir flood-operation workflows. The methodology converts natural-language operational intent into structured workflow execution, while limiting the LLM role to instruction interpretation, workflow coordination, and structured reporting. Second, it develops an evidence-bound and fail-closed validation mechanism for LLM-assisted reservoir-operation records. A decision is accepted only when the required workflow is completed, the final payload is bound to the current tool-returned evaluation result, the schema is valid, and hard operational constraints are satisfied. Third, it validates the methodology through a real-event Tankeng Reservoir case study covering static planning, dynamic retain/replan operation, command intervention, rolling forecast-triggered operation, forecast-error audit, and tools/skills ablation. The results show that text-only generation and tools-only execution are insufficient for auditable reservoir-operation records, whereas deterministic tools combined with workflow protocols and external validation can support reliable LLM-assisted execution under the evaluated single-reservoir setting.

---

# 2. Related Work

## 2.1 Reservoir operation as constrained engineering decision support

Reservoir operation has a long history in water-resources systems analysis. Early state-of-the-art reviews emphasized that reservoir-management models must balance physical water balance, engineering constraints, operating rules, and competing objectives (Yeh, 1985; Labadie, 2004). Rule curves and operating rules remain widely used because they are interpretable, familiar to operators, and easy to apply under routine conditions. Optimization methods, including dynamic programming, nonlinear programming, evolutionary algorithms, and multi-objective search, provide more flexible ways to compute release schedules when objectives and constraints can be explicitly specified.

Rolling-horizon and model-predictive-control formulations address the fact that flood operation is sequential and forecast-dependent. In these methods, the release plan is recomputed as updated state or forecast information becomes available. Recent studies have explored scenario-based MPC and dynamically parameterized MPC for reservoir systems under uncertain inflow and operator preference changes (Cestari et al., 2023; Koo et al., 2024). These studies highlight a central issue also relevant to LLM-assisted reservoir operation: practical operation involves not only release optimization but also forecast uncertainty, changing operator intent, runtime feasibility, and the ability to revise decisions as the event evolves.

The present study does not introduce a new reservoir-optimization algorithm. The deterministic operation kernel is used as a reproducible computational substrate. The research question is whether an LLM agent can enter the reservoir-operation workflow as a coordinator without breaking the traceability and constraint discipline required for engineering decision support. The unit of analysis is therefore not only a release trajectory or objective value, but a decision record that includes the scenario state, tool trace, evaluation reference, structured payload, and validation outcome.

## 2.2 Expert systems and auditable water-resources decision workflows

Expert systems were developed to make domain knowledge explicit through rules, inference procedures, and explanation facilities (Hayes-Roth et al., 1983; Feigenbaum, 1986). In engineering decision support, this principle is important because operators need to understand how a recommendation was produced and whether it follows approved procedures. Water-resources applications have used expert-system ideas for rule-based operation, monitoring, diagnosis, and planning, where the traceability of a recommendation is often as important as the numerical result.

The same requirement becomes sharper when LLM agents are introduced. A language model can make a system easier to use through natural-language interaction, but it does not by itself provide an approved operating procedure. In a reservoir-operation setting, domain knowledge must be represented not only as facts or rules, but also as executable workflow constraints: which tools should be called, in what order, under which branch conditions, and with which evidence in the final record. This view connects LLM-assisted operation with the expert-system tradition. The agent-facing workflow must preserve explicit procedural knowledge, auditable traces, and externally checkable outcomes.

In this paper, the expert-system element is not a free-form chain of reasoning produced by the LLM. It is the combination of deterministic tools, workflow protocols, evidence-bound payloads, and fail-closed validation. The LLM is useful because it can interpret operator instructions and coordinate execution, but the acceptability of a decision depends on whether the workflow record satisfies verifiable engineering rules.

## 2.3 LLM agents for tool-mediated engineering tasks

Tool-augmented LLMs connect language models to external resources such as calculators, search engines, databases, code interpreters, and domain-specific functions. MRKL systems proposed a modular neuro-symbolic architecture in which language models are complemented by external knowledge and reasoning modules (Karpas et al., 2022). ReAct showed that reasoning traces and actions can be interleaved so that a language model can interact with external environments and update its behavior based on observations (Yao et al., 2023). Toolformer studied how models can learn to call APIs for tasks such as arithmetic and factual lookup (Schick et al., 2023). These lines of work support the general premise that LLMs become more useful when they can access external computation.

For engineering applications, however, tool use should not be equated with reliable execution. A model may decide not to call a required tool, call correct tools in an invalid order, or produce a final response that is not tied to the result returned by the tool. These failures may be tolerable in low-stakes information tasks, but they are problematic in reservoir flood operation because they affect whether the final record can be reproduced and audited. Tool access gives the agent computational reach; it does not automatically define the permitted operational procedure.

The present study therefore treats tool-mediated LLM execution as a workflow-control problem. The key requirement is not only that the model can call a reservoir-operation tool, but that the resulting decision remains inside a specified business process. This distinction motivates the separation between text-only generation, tools-only execution, and tools combined with workflow skills and validation in the ablation experiment.

## 2.4 From tool access to workflow-constrained agent execution

Standardized tool interfaces make it easier to connect LLM agents to external systems. The Model Context Protocol was introduced as an open standard for connecting AI applications to files, databases, tools, and specialized workflows (Anthropic, 2024). Such standardization is useful for engineering applications because domain functions can be exposed through structured inputs and outputs rather than ad hoc prompts. Emerging work on MCP servers and MCP-based agents also shows that tool access creates new safety, maintainability, and verification concerns, including tool poisoning risks and expanded attack surfaces (Hasan et al., 2025; Radosevich and Halloran, 2025).

For reservoir operation, the main issue is hydrological auditability rather than cybersecurity. A standardized tool interface can expose the deterministic operation kernel to the agent, but the agent still needs workflow constraints that determine which tool sequence is valid for a given operational context. Static planning, dynamic retain/replan operation, and rolling forecast-triggered operation do not require the same sequence of calls. A valid tool in one branch may be prohibited in another branch. This means that the workflow protocol is part of the engineering knowledge base, not a minor implementation detail.

PyResOps instantiates this idea by pairing tool access with explicit workflow skills. These skills specify required chains, branch logic, prohibited calls, evidence-reference rules, and final payload formats. The design places the LLM in a controlled coordination role and makes workflow compliance an object of validation. This is the methodological step that moves beyond ordinary tool availability.

## 2.5 Auditability and fail-closed validation in safety-relevant AI systems

AI-assisted engineering systems require evaluation criteria that differ from ordinary natural-language generation. In many text tasks, a response can be judged by semantic adequacy or user preference. In safety-relevant decision support, the response must also be reproducible, evidence-bound, and constraint-checked. A safe-looking answer without a traceable computation path is not acceptable as an operation record.

Fail-closed validation provides a conservative acceptance logic for this setting. If a required tool call, evaluation reference, schema field, or constraint check is missing or invalid, the decision is rejected. Rejection is therefore part of the safety mechanism, not merely a performance failure. In LLM-assisted reservoir operation, this criterion changes the meaning of success: an accepted record must demonstrate both hydrological feasibility and workflow auditability.

This paper contributes to this line of work by applying fail-closed validation to LLM-assisted single-reservoir flood operation. The results are interpreted at the decision-record level. Accepted and rejected records are both informative because they reveal whether the agent remained inside the permitted workflow. The ablation experiment further tests the mechanism by comparing free text, tool access without explicit workflow skills, and full workflow-constrained execution.

---

# 3. Methodology: workflow-constrained LLM-assisted reservoir operation

## 3.1 Methodological overview: embedding LLM agents into reservoir-operation workflows

The proposed methodology embeds LLM agents into reservoir flood operation as workflow coordinators rather than reservoir-operation decision makers. The operational chain is organized as instruction interpretation, task formalization, deterministic tool execution, evidence binding, external validation, and audit-record generation. This chain defines how a natural-language instruction can enter a safety-relevant engineering workflow without allowing the language model to become the source of numerical authority.
...
**Figure 1. Workflow-constrained LLM-assisted reservoir-operation methodology.**  
`[TO FILL: architecture figure. Suggested panels: operator instruction; task formalization; deterministic reservoir-operation kernel; tool interface; LLM-agent workflow coordinator; workflow skills; evidence-bound decision payload; fail-closed validator; accepted/rejected audit record. Show that the LLM does not compute hydraulic quantities directly and that validation occurs after tool execution.]`

The separation between deterministic computation and agent coordination is central to the methodology. All quantities that determine physical feasibility are computed by tools. The LLM is not asked to estimate water levels, storage changes, release capacities, objective components, or constraint violations from text. Its role is limited to interpreting instructions, selecting the appropriate workflow, coordinating permitted tools, and producing a structured final payload. The final payload is accepted only if it can be linked to the current tool-returned evidence and passes external validation.
...
## 3.2 Reservoir-operation task formalization and deterministic grounding
...
## 3.3 Workflow-constrained agent execution
...
## 3.4 Evidence-bound decision records and fail-closed validation
...
This criterion converts LLM-agent evaluation into decision-record validation. The validator checks whether the workflow was completed, whether the final record is bound to the current tool-returned evidence, whether required fields are present and valid, and whether hard operational constraints are satisfied. Rejected records are logged with failure categories, expected tool chains, observed tool chains, and reference-binding status. These logs support the failure taxonomy reported in Section 5.4 and Appendix J.

## 3.5 PyResOps implementation

PyResOps implements the methodology for the Tankeng Reservoir single-reservoir flood-operation case study. It consists of a deterministic reservoir-operation kernel, an MCPTools interface, an Agno-based LLM-agent execution layer, workflow skill protocols, and a fail-closed validation layer. The deterministic kernel is organized into modules for event loading, reservoir state update, release-family construction, optimization, simulation, evaluation, and validation. The FastMCP/MCPTools layer wraps these functions as structured tools and returns machine-readable outputs with stable result identifiers.
...
## 4.1 Study site and operational constraints

The case study uses processed flood-event records from Tankeng Reservoir in Fujian Province, China. Tankeng Reservoir is a large multipurpose reservoir with flood-control, hydropower, water-supply, and ecological-flow functions. Flood-control operation is the relevant setting in this paper. Figure 2 provides a study-area and operational-context overview.
...
This separation prevents confusion between engineering background information and experimental validation constraints. PyResOps is evaluated as an auditable decision-support layer under the specified scenario constraints. It is not presented as an official operating rule or as a replacement for reservoir-authority review.

## 4.2 Flood-event dataset and scenario groups
...
## 4.3 Three-stage validation design

The evaluation uses three validation stages to support the proposed methodology. Stage 1 directly calls the deterministic reservoir-operation kernel through code. No MCPTools, Agno workflow, or LLM executor is involved. This stage validates scenario executability, constraint evaluation, and deterministic reference generation.
...
**Figure 3. Stage 1-2-3 validation design and execution summary.**  
`[TO FILL: execution-summary figure. Suggested panels: Stage 1 direct code, Stage 2 Agno+MCPTools, Stage 3 Agno+LLM+skills+MCPTools; record pools: 462 main workflow set, 492 static instruction matrix, 40 command subset, 51 forecast-error audit, ablation; acceptance/failure summary by executor.]`

## 4.4 Evaluation blocks
...
## 5.1 Static instruction-conditioned release planning

The static scenario evaluates whether PyResOps can generate feasible full-horizon release plans while following operator-specified release forms and implementation intervals. The default static workflow accepted all 41 retained events in Stage 1 and Stage 2, with zero hard-constraint violations and zero downstream-release violations.
...
## 5.2 Dynamic retain/replan and command-intervention operation
...
The representative event 2024061623 illustrates how command interventions modify remaining-horizon operation. A release-cap command at the inflow-peak checkpoint remains feasible under the constrained release plan. A conservative risk-buffer command at the same checkpoint imposes a stricter upper-level target and produces a different release trajectory. These examples show that PyResOps can translate distinct operator intentions into different constrained release plans while keeping the final decision auditable.
...
## 5.3 Rolling forecast-triggered operation and forecast-error audit
...
## 6.1 LLM agents as workflow coordinators rather than reservoir-operation decision makers

The results support a bounded interpretation of how LLM agents can enter reservoir flood-operation workflows. The useful role of the agent in this study is not to compute hydraulic quantities or to replace reservoir-operation models. Its role is to interpret operator instructions, select the relevant workflow, coordinate deterministic tools, and generate a structured decision payload that can be externally checked. This distinction is central to the proposed methodology. In a safety-relevant engineering setting, the language model should not be treated as the source of numerical authority; the authority comes from deterministic computation and validation records.

The three-stage validation design reinforces this role boundary. Stage 1 verifies that the reservoir-operation task can be executed by direct code. Stage 2 verifies that the same computation can be reproduced through tool-mediated workflows. Stage 3 then tests whether an LLM executor can coordinate this workflow without violating the protocol. This separation prevents tool-interface errors, kernel errors, and agent-execution errors from being conflated. It also provides a practical development route for LLM-assisted engineering decision systems: deterministic computation should be validated first, tool-mediated execution should be checked second, and LLM-agent coordination should be evaluated only after these two layers are stable.

This framing also changes how rejected records should be interpreted. A rejected record may correspond to a numerically safe release plan, but it cannot be accepted if the evidence trail is invalid. Conversely, a fluent final answer is not useful if it cannot be traced to the current tool-returned evaluation result. The failure taxonomy therefore provides more information than an aggregate acceptance rate alone. It shows whether the agent remained inside the permitted workflow and whether the final decision record can be reconstructed after the event.

## 6.2 Workflow-constrained execution across static, dynamic, command, and rolling operation

The evaluated workflows represent different entry points through which an LLM agent may participate in reservoir-operation business processes. Static instruction-conditioned planning tests whether the agent can follow a specified release family and implementation interval. The deterministic 492-record matrix establishes that the requested combinations are executable under the evaluated constraints, while the Stage 3 stratified subset tests whether LLM executors can follow the same instruction-conditioned workflow and bind the final answer to valid evidence.

Dynamic retain/replan operation exposes a different form of workflow risk. The agent must choose between branches with different permitted tool chains. In a retain stage, unnecessary preparation or optimization calls violate the workflow protocol; in a replan stage, the full chain must be completed. This explains why dynamic operation is a stricter test of workflow discipline than a linear static workflow. The command-intervention subset adds mid-event operator instructions, including release-cap adjustment, terminal-target lowering, deadline compression, and conservative risk buffering. These records evaluate whether natural-language operational intent can be translated into updated constraints and remaining-horizon execution.

Rolling forecast-triggered operation tests temporal auditability. The workflow does not call the LLM at every 3 h check. Instead, it records deterministic retain rows when no trigger fires and calls the LLM-mediated replan workflow only under trigger conditions. The 61.9% LLM-call reduction should not be read as a universal efficiency claim because it depends on trigger thresholds, forecast quality, and event characteristics. Its methodological significance is that selective agent participation can coexist with full audit coverage. The forecast-error audit further shows how forecast deviations can be treated as workflow triggers rather than asking the LLM to infer hydrological risk directly from text.

Across these workflows, the common feature is that the agent operates inside a specified business process. Static, dynamic, command-intervention, and rolling experiments do not simply test whether a model can produce plausible answers. They test whether different forms of reservoir-operation interaction can be converted into executable and auditable decision records.

## 6.3 Why tool access alone is insufficient for auditable reservoir operation
...
Tools-only execution improved computational grounding but remained vulnerable to protocol and evidence-binding failures. The agent had access to the necessary computations, but tool availability did not ensure that the tools were called in the correct order, that required steps were completed, or that the final payload cited the current evaluation result. This result is important because it separates tool access from workflow reliability. Engineering auditability requires not only the ability to call a tool, but also a verified procedure for using that tool.

The tools + workflow skills / full MCP setting accepted all records in the ablation subset. This does not imply that the implementation is universally reliable, but it supports the mechanism that the paper tests: deterministic tools, workflow protocols, evidence binding, and fail-closed validation jointly reduce the space of acceptable agent behavior. Under this design, the system does not need to trust the LLM's wording. It accepts only records that can be traced to a valid tool execution and constraint check.

This finding is relevant to expert-system research. The expert knowledge is not only embedded in the reservoir equations or tool functions. It is also embedded in the workflow protocol: which branch is allowed, which tool sequence is required, which evidence must be cited, and when a record must be rejected. The LLM-facing interface becomes useful only when this procedural knowledge is explicit and externally validated.

## 6.4 Scope, development implications, and future extensions

Several boundaries define the scope of the present validation. The experiments are conducted on a single reservoir, Tankeng Reservoir, using 41 retained flood events and the operating constraints configured for this case study. The results support the feasibility of workflow-constrained LLM-assisted execution under the evaluated single-reservoir setting, but they do not establish general operational performance across reservoirs with different storage-elevation curves, outlet-capacity functions, downstream protection standards, dispatch rules, or flood-season control policies. The scenario-imposed upper operating level of 160.0 m and the uniform downstream release constraint of 3000 m³/s are case-specific validation settings.
